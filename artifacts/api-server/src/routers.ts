    ...
    calculate: protectedProcedure
      ...
      .query(async ({ input, ctx }) => {
        ...
        const memberPrefs = prefRows.map((p) => ({
          ...
          // Extended v2 fields
          noisePreference: p.noisePreference ?? null,
          energyLevel: p.energyLevel ?? null,
          avoidVenues: safeParseJSON(p.avoidVenues),
          prioritizeType: p.prioritizeType ?? null,
        }));
  ...
  calendar: router({
    ...
    updatePreferences: protectedProcedure
      .input(z.object({
        ...
        // Extended v2 preferences
        noisePreference: z.string().optional(),
        energyLevel: z.number().min(1).max(5).optional(),
        flexibilityLevel: z.number().min(1).max(5).optional(),
        idealDuration: z.number().optional(),
        avoidRain: z.boolean().optional(),
        outdoorOnlyGoodWeather: z.boolean().optional(),
        prefersSunny: z.boolean().optional(),
        midpointPreference: z.boolean().optional(),
        prioritizeType: z.string().optional(),
        avoidVenues: z.array(z.string()).optional(),
      ...
      .mutation(async ({ ctx, input }) => {
        await db.upsertUserPreferences(ctx.user.id, {
          ...
          vibes: input.vibes !== undefined ? JSON.stringify(input.vibes) : undefined,
          cuisines: input.cuisines !== undefined ? JSON.stringify(input.cuisines) : undefined,
          dietaryRestrictions: input.dietaryRestrictions !== undefined ? JSON.stringify(input.dietaryRestrictions) : undefined,
          foodPreferences: input.foodPreferences !== undefined ? JSON.stringify(input.foodPreferences) : undefined,
          preferredDays: input.preferredDays !== undefined ? JSON.stringify(input.preferredDays) : undefined,
          meetupTimes: input.meetupTimes !== undefined ? JSON.stringify(input.meetupTimes) : undefined,
          ...
          avoidRain: input.avoidRain,
          outdoorOnlyGoodWeather: input.outdoorOnlyGoodWeather,
          prefersSunny: input.prefersSunny,
          midpointPreference: input.midpointPreference,
          prioritizeType: input.prioritizeType,
          avoidVenues: input.avoidVenues !== undefined ? JSON.stringify(input.avoidVenues) : undefined,
          accessibilityNeeds: input.accessibilityNeeds !== undefined ? JSON.stringify(input.accessibilityNeeds) : undefined,
        });
        return { success: true };
      }),
